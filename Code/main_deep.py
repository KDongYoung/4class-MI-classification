...
from models import DeepConvNet_ch
...
def Experiment(args, subject_id, alpha):
    ...
    if not os.path.isdir(path):
        ...
        os.makedirs(args.save_root + '/Prediction_DeepConvNet')
    ...
    model = DeepConvNet_ch(n_classes, n_channels, n_frequency)
    if cuda:
    ...
    dataloaders = DataGenerator(args, subject_id+1)
    test_loader = dataloaders.test_loader
    best_model = DeepConvNet_ch(n_classes, n_channels, n_frequency)
    ...
    np.save(args.save_root + '/Prediction_DeepConvNet/S0'+str(subject_id+1), pred)
...
def main(args, alpha):
    if alpha==0:
        exp_type="DeepConvNet"
    else:
        exp_type="DeepConvNet_align"+str(alpha)
